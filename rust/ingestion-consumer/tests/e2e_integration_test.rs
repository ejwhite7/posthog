...
impl Harness {
    ...
    /// Ledger commit mode at poll granularity: the Kafka commit comes from the
    /// ledger frontier instead of the batch spans, and a whole poll completes
    /// before anything commits.
    async fn start_commit_mode(
        topic: &str,
        partitions: i32,
        worker_count: usize,
        max_in_flight: usize,
    ...
    /// Ledger commit mode at group granularity: each send completes on its own
    /// and commits its partitions' frontiers, so a stalled send holds only the
    /// partitions it sits on.
...
async fn commit_mode_commits_the_ledger_frontier() {
    ...
    assert_commits_track_the_ledger_frontier(Harness::start_commit_mode(&topic, 1, 1, 1).await)
        .await;
}
...
async fn poll_granularity_holds_both_partitions_while_one_stalls() {
    ...
    let harness = Harness::start_commit_mode(&topic, 2, 2, 2).await;
    let mut guards = stall_two_partitions_on_two_workers(&harness, &topic).await;
