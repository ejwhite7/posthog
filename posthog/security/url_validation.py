...
def _blocked_host_reason(host: str) -> str | None:
    ...
    IP: split-horizon DNS, and a registered domain shaped like an internal one. Pass a
    lowercased host.
    """
...
def _external_target_check_disabled() -> bool:
    """SSRF host/URL validation is skipped in local dev and the test suite.

    Destinations there legitimately point at localhost or private hosts (a local
    Postgres, the dev object store), so validation would block routine setup. Set
    FORCE_URL_VALIDATION (POSTHOG_FORCE_URL_VALIDATION) to run the production path
    locally.
    """
    return _dev_bypass_enabled() or (settings.TEST and not settings.FORCE_URL_VALIDATION)
...
def validate_external_url(url: str) -> None:
    ...
    a webhook). No-op in dev/test, see ``_external_target_check_disabled``.
    """
    if _external_target_check_disabled():
        return
...
def validate_external_host(host: str) -> None:
    ...
    cannot drift apart. No-op in dev/test, see ``_external_target_check_disabled``.
    """
    if _external_target_check_disabled():
        return
    host = host.lower()
